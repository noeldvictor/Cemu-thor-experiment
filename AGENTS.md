...
The built-in Star Fox Zero USA profile is `bin/gameProfiles/default/00050000101b0400.ini`. It uses multi-core recompiler, a modestly larger thread quantum, async compile, accurate barriers on, and GX2DrawDone full sync off. **The GX2DrawDone setting is inert on this build**: `GX2DrawDone()` in `src/Cafe/OS/libs/gx2/GX2_Event.cpp` forces the full async-operation sync whenever the renderer is Vulkan, before it ever looks at `ActiveSettings::WaitForGX2DrawDoneEnabled()`, so neither the profile key nor the OSD "skip GX2DrawDone sync" toggle changes anything on Android (found by the 2026-09-07 code review). Do not describe it as a measured win. Making the toggle real means deciding whether the Vulkan readback/occlusion paths can tolerate skipping the sync, which is a correctness question, not a flag flip. Game profiles can override `asyncCompile`, `accurateBarriers`, and `gx2DrawDoneSync`; runtime code should read these through `ActiveSettings` instead of directly from `GetConfig()` when emulation is active.
